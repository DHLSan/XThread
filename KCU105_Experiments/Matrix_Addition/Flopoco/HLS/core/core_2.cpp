...
#pragma HLS PIPELINE II = 1
    	...
    	loop_b:for(int j=0;j<matrix_size/16;j++){
    		local_mem_flopoco = getLocalLineFlopoco(mem,matrix_B_offset + i*matrix_size/16 + j);
    	       for(int k=0;k<16;k++){
    	    		local_b_row[j*16 + k] = local_mem_flopoco[k];
    	   		}
    	}
...
#pragma HLS PIPELINE II = 1
    		  local_c_row[j] =thls::add_dual<E,F>(local_a_row[j],local_b_row[j],0);
    	}

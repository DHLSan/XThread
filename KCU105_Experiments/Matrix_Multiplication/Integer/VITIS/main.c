...
int *mem_float = (int *) 0x80000000; // write purpose
...
//change as 16  to run Xthread hardware
//change as 2  to run microblaze
...
void run_mm(int core_number, int *mem){
    ...
    for (int i = start; i < end; i++) {
        for (int j = 0; j < matrix_size; j++) {
            ...
            for (int k = 0; k < matrix_size; k++) {
                temp += mem[matrix_A_offset + (size_i * i) + k] * mem[matrix_B_offset + (size_i * k) + j];
            }
            mem[matrix_C_offset + (size_i * i) + j] = temp;
        }
    }

}
void run_bmm(int core_number,int *mem){
    			...
    			for (i = 0; i < matrix_size; ++i){
    				for (j= jj; j < jj + BLOCK_SIZE; ++j){
    					for (k = kk; k < kk + BLOCK_SIZE; ++k){
    						mem[MATRIX_C_OFFSET+i*matrix_size + j] += mem[MATRIX_A_OFFSET+i*matrix_size+k] * mem[MATRIX_B_OFFSET+k*matrix_size+j];
    					}
    				}
    			}
    		}

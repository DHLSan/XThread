...
void writeLocalLine(volatile ap_uint<512> *mem, float local_int_mem[16], int lineNumber){
#pragma HLS inline
	ap_uint<512> local_line;
	for(int i=0;i<16;i++){
#pragma HLS UNROLL FACTOR=16
		float ftemp = local_int_mem[i];
...
#pragma HLS INTERFACE s_axilite port=control_unit bundle=BUS_SLAVE
	...
	// read first 16 float
	float local_mem_s[16];
//#pragma HLS array_partition variable=local_mem_s complete dim=1
	float* local_mem= &local_mem_s[0];
...
//#pragma HLS array_partition variable=local_b cyclic factor=4  dim=2
...
//#pragma HLS array_partition variable=local_c cyclic factor=2 dim=1
